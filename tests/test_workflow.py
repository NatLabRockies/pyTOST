...
def test_workflow_cluster_returns_primary_and_bootstrap(basic_diff_df):
    out = run_tost(
        ...
        engine="cluster",
        cluster="cluster_id",
        options=WorkflowOptions(do_sensitivity=False, bootstrap_B=10, seed=123, cross_cluster_dependence=False),
    )
...
def test_workflow_spatial_smoke(spatial_df):
    ...
    assert "primary" in out
    assert "bootstrap" in out
    assert out["bootstrap"]["method"] in {"cluster_bootstrap", "spatial_within_cluster_block_bootstrap", "spatial_block_bootstrap"}
