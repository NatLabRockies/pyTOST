...
pyTOST requires **Python 3.10+**.

Base install from a local clone:
...
Optional R-backed mixed-effects functionality in the spatial engine:

```bash
pip install -e ".[r]"
```

Full maintainer install:

```bash
pip install -e ".[test,r]"
```

The base install does **not** require R. Optional R-backed functionality requires a working **R** installation that `rpy2` can bind to, along with any needed R packages such as `lme4` and `lmerTest`.

pyTOST currently declares the following base runtime dependencies:
...
Some optional functionality in the spatial engine becomes more complete when R-related tooling is available, but the core library workflow remains `run_tost(...)`.
