...
diagnostics.py
===============
Shared diagnostics to detect dependence structures (cluster, spatial, temporal)
to guide TOST engine selection.

References
----------
- Moran (1950). Biometrika 37:17–23.
- Cliff & Ord (1981). Spatial Processes: Models & Applications. Pion.
- Cressie (1993). Statistics for Spatial Data. Wiley.
- Snijders & Bosker (2012). Multilevel Analysis. Sage.
- Bell & McCaffrey (2002). Survey Methodology 28:169–182.
- Pustejovsky & Tipton (2018). J. Bus. & Econ. Stats. 36(4):672–683.
- Box & Pierce (1970) / Ljung–Box variant (1978). Biometrika.
"""
...
def intraclass_correlation(df: pd.DataFrame, y: str, cluster: str) -> float:
    """
    One-way ANOVA ICC: tau^2 / (tau^2 + sigma^2), method-of-moments.

    Notes
    -----
    MSB = sigma^2 + kbar*tau^2, MSW = sigma^2 (Snijders & Bosker, 2012).
    """
...
def se_inflation_ratio(se_cluster: float, se_iid: float) -> float:
    """> 1 ⇒ IID SE is too small relative to cluster-aware SE."""
    if se_iid <= 0:
        return np.inf
...
def morans_I(df: pd.DataFrame, y: str, x: str, ycoord: str, k: int = 4) -> Tuple[float, float]:
    """
    Moran's I with k-NN row-standardized weights (quick approximation).
    Returns (I, p-value via rough normal approximation).
    """
...
def durbin_watson(resid: np.ndarray) -> float:
    """DW statistic for AR(1)-type autocorrelation in ordered residuals."""
...
def summarize_diagnostics(df: pd.DataFrame,
    ...
    """
    Compute a common set of diagnostics used by the orchestrator to choose engines.
    Returns keys among: icc, se_inflation, morans_I, morans_p, dw, ljung_box_Q, ljung_box_p
    """
    out: Dict[str, float] = {}
